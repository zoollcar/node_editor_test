...
static void
node_editor_push(struct node_editor *editor, struct node *node) /* 追加 */
{
    if (!editor->begin) {
        node->next = NULL;
        node->prev = NULL;
...
static void
node_editor_pop(struct node_editor *editor, struct node *node) /* 提取 */
{
    if (node->next)
...
node_editor_init(struct node_editor *editor)
{
    ...
    editor->show_grid = nk_false;
}

static int
my_node_editor(struct nk_context *ctx) /* 主要函数 */
{
    int n = 0;
        ...
        nk_layout_space_begin(ctx, NK_STATIC, total_space.h, nodedit->node_count);
        {
            ...
            if (nk_contextual_begin(ctx, 0, nk_vec2(100, 220), nk_window_get_bounds(ctx))) {
                ...
                if (nk_contextual_item_label(ctx, "New", NK_TEXT_CENTERED))
                    node_editor_add(nodedit, "New node", nk_rect(400, 260, 180, 220),
                            nk_rgb(255, 255, 255), 2, 3);
